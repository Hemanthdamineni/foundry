...
from sdlc.cli.merger import deep_merge, merge_opencode_json
...
class TestDeepMerge:
    ...
    def test_list_append_dict_items(self) -> None:
        base = {"agents": [{"name": "dev-sdlc", "mode": "primary"}]}
        new = {"agents": [{"name": "reviewer", "mode": "secondary"}]}
        deep_merge(base, new)
        assert len(base["agents"]) == 2
        assert base["agents"][1]["name"] == "reviewer"
    ...
    def test_deep_nested(self) -> None:
        base = {"mcpServers": {"sdlc": {"command": "foo"}}}
        deep_merge(base, {"mcpServers": {"sdlc": {"args": ["--dev"]}}})
        assert base["mcpServers"]["sdlc"] == {"command": "foo", "args": ["--dev"]}
...
class TestMergeOpencodeJson:
    def test_creates_new_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fragment = {"mcpServers": {"test": {"command": "foo"}}}
            changed = merge_opencode_json(target, fragment)
            assert changed is True
            assert target.exists()
            data = json.loads(target.read_text())
            assert data["mcpServers"]["test"]["command"] == "foo"
    ...
    def test_merge_mcp_server_entry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            data = json.loads(target.read_text())
            srv = data["mcpServers"]["sdlc-orchestrator"]
            assert srv["command"] == "sdlc-mcp"

    def test_merge_preserves_user_agents(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "opencode.json"
            target.write_text(json.dumps({"agents": {"user-agent": {"mode": "primary"}}}))
            fragment = {"agents": {"dev-sdlc": {"mode": "primary"}}}
            merge_opencode_json(target, fragment)
            data = json.loads(target.read_text())
            assert "user-agent" in data["agents"]
            assert "dev-sdlc" in data["agents"]
