...
1. `sdlc_detect_workspace` with no path
2. `sdlc_bootstrap_workspace` if the selected workspace is not ready
3. `sdlc_create_task`
4. `sdlc_get_next_action`

The runtime owns `.sdlc` infrastructure. Do not hand-generate `.sdlc` files.
When detecting a workspace, read and respect the returned `execution_root`,
`workspace_root`, and `detection_reason` before proceeding. Do not pass parent
paths or guessed repository roots into MCP tools; workspace overrides must come
from the runtime CLI `--workspace` flag or the `FOUNDRY_WORKSPACE` environment
variable.
