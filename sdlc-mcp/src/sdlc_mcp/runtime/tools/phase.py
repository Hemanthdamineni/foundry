...
from sdlc_mcp.engine.orchestrator import OrchestratorError
...
async def get_next_action(  # noqa: PLR0913 - tool wiring keeps runtime dependencies explicit.
    ...
    return {
        ...
        "subagent": route.get("subagent", "foundry"),
        "model": route.get("model", model_routing.get("default_model", "opencode/current")),
        "model_source": route.get("model_source", model_routing.get("model_source", "opencode")),
...
async def submit_output(  # noqa: C901, PLR0912, PLR0913
    ...
    if schema_violations:
        record = PhaseRecord(
            ...
            output=output,
            error="Deterministic schema checks failed",
        ...
        return {
            ...
            "hint": "Fix the phase output and resubmit.",
        }
