...
from sdlc_mcp.bootstrap.workspace import resolve_workspace
...
class LLMRoutingConfig(BaseModel):
    ...
    @classmethod
    def accept_nested_role_config(cls, data: Any) -> Any:
        """Accept either flat keys or role: {provider, model} YAML."""
        if not isinstance(data, dict):
            return data
        result = dict(data)
        for role in ("judge", "debate_agent", "debate_consensus"):
            nested = result.get(role)
            if isinstance(nested, dict):
                provider = nested.get("provider")
                model = nested.get("model")
                if provider is not None:
                    result[f"{role}_provider"] = provider
                if model is not None:
                    result[f"{role}_model"] = model
        return result
...
class LLMConfig(BaseModel):
    ...
    default_model: str = "opencode/current"
...
class Settings(BaseSettings):
    ...
    db_path: str = "workspace.db"
    plugin_state_dir: str = "plugin_state"
    checkpoint_dir: str = "checkpoints"
    log_path: str = "logs/sdlc.log"
    ...
    def workspace_root(self) -> Path:
        """Return the workspace root this runtime owns for the current process."""
        configured = os.environ.get("FOUNDRY_WORKSPACE")
        if configured:
            return Path(configured).expanduser().resolve()
        return resolve_workspace().workspace_root

    def sdlc_dir(self) -> Path:
        """Return the per-workspace SDLC state directory."""
        return self.workspace_root() / ".sdlc"

    def resolve_runtime_path(self, path: str | Path) -> Path:
        candidate = Path(path).expanduser()
        if candidate.is_absolute():
            return candidate
        return self.sdlc_dir() / candidate

    def resolve_config_path(self, filename: str) -> Path:
        config_dir = self.resolve_runtime_path(self.config_dir)
        candidates = [
            config_dir / filename,
            self.workspace_root() / "config" / filename,
            self.workspace_root() / "configs" / filename,
            PACKAGE_ROOT / "config" / filename,
            PACKAGE_ROOT / "configs" / filename,
            SOURCE_ROOT / "config" / filename,
            SOURCE_ROOT / "configs" / filename,
        ]
    ...
    def phase_graph_path(self, name: str = "feature") -> Path:
        candidates = [
            self.resolve_runtime_path(self.config_dir) / "graphs" / f"{name}.yaml",
            PACKAGE_ROOT / "graphs" / f"{name}.yaml",
            SOURCE_ROOT / "graphs" / f"{name}.yaml",
    ...
    def load_llm_config(self) -> LLMConfig:
        ...
        if path.exists():
            ...
            if isinstance(data.get("llm"), dict):
                data = cast("dict[str, Any]", data["llm"])
