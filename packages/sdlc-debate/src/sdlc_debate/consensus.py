...
Adapted from Helix/foundry/sdlc/engine/consensus.py.
"""
...
from sdlc_models.debate import CollapseSignal, ConsensusResult, MinorityReport
...
class ConsensusEngine:
    ...
    def extract_minority_reports(
        ...
        """Extract minority reports from agents whose verdict was negative."""
    ...
    def detect_collapse(self, responses: dict[str, str]) -> CollapseSignal:
        """Detect sycophantic collapse via keyword matching.

        Returns a CollapseSignal with confidence 0.0 if no collapse detected,
        or a signal with detected=True and a confidence ratio otherwise.
        """
    ...
    def extract_residual_objections(
        ...
        """Find objections raised in round 0 that persist (still/remains) in the final round."""
    ...
    async def _llm_consensus(
        ...
        """Use an LLM judge to evaluate consensus from agent responses."""
        agent_lines = [
    ...
    def _extract_verdict_from_text(text: str) -> bool | None:
        """Heuristic: scan for PASS/FAIL tokens in free-text response."""
        tokens = {
            token.strip(".,:;!?()[]{}'\"-").lower()
    ...
    def _majority_consensus(
        ...
        """Fallback: majority-vote consensus when the LLM judge is unavailable."""
        agent_verdicts: dict[str, bool] = {}
        for role, response in responses.items():
