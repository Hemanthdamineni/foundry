...
Adapted from Helix/foundry/sdlc/engine/debate_runtime.py.
"""
...
def _agent_roles_for_phase(phase: str) -> list[DebateAgentRole]:
    """Map an SDLC phase name to the set of agent roles that deliberate on it."""
...
class DebateRuntime:
    ...
    Does NOT access filesystem or external state (except LLM via the provider).

    Parameters
    ----------
    provider:
        An LLMProvider implementation (e.g. OllamaProvider).
    model:
    ...
    async def run_debate(
        ...
        """Execute the full multi-round debate protocol for a phase output.

        Returns a ``DebateTranscript`` that captures every round, the final
        consensus result, minority reports, collapse signals, and a token
        estimate.  When debate is disabled by budget policy or no agents are
        configured for *phase*, a no-op transcript with ``consensus.reached=True``
        ...
        for round_num in range(max_rounds):
            ...
            if consensus.collapse_signal.detected and round_num == max_rounds - 1:
                pass  # Force consensus with warning — handled by returning the transcript.
    ...
    async def _call_agent_with_retry(
        ...
        Returns the agent's response text, or an error string prefixed with
        ``ERROR after N attempts: ...`` when all retries are exhausted.
        """
    ...
    async def _call_agent(
        ...
        """Construct the prompt for one agent in one round and call the LLM."""
    ...
    def _build_minority_reports(
        ...
        """Populate minority reports from the consensus result or fall back to heuristic."""
    ...
    def _detect_collapse_multi_round(
        ...
        """Aggregate collapse signals across rounds; return the worst if any."""
    ...
    def _skip_transcript(
        ...
        """Return a no-op transcript with consensus pre-set to passed."""
    ...
    def _estimate_tokens(self, transcript: DebateTranscript) -> int:
        """Rough token estimate: count whitespace-separated tokens and double."""
        total = 0
