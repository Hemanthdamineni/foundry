...
that must be satisfied before the output is considered structurally valid.
"""
...
class FailureType(enum.StrEnum):
    """Broad failure categories used by the SDLC engine."""
...
def _has_min_content(output: str, min_lines: int = 3) -> bool:
    """Return True if the output has at least *min_lines* non-blank lines."""
...
def check_specs_output(output: str) -> list[SchemaViolationError]:
    """Validate Specs phase output has required requirement/scope/constraint sections."""
    violations: list[SchemaViolationError] = []
...
def check_planning_output(output: str) -> list[SchemaViolationError]:
    """Validate Planning phase output has required section structure."""
    violations: list[SchemaViolationError] = []
...
def check_coding_output(output: str) -> list[SchemaViolationError]:
    """Validate Coding phase output references specific files."""
    violations: list[SchemaViolationError] = []
    found_any = any(
...
def check_review_output(output: str) -> list[SchemaViolationError]:
    """Validate Review phase output has required issue/severity/fix sections."""
    violations: list[SchemaViolationError] = []
...
def check_testing_output(output: str) -> list[SchemaViolationError]:
    """Validate Testing phase output has required results/coverage/failure sections."""
    violations: list[SchemaViolationError] = []
...
def check_done_output(output: str) -> list[SchemaViolationError]:
    """Validate Done phase output includes a summary."""
    violations: list[SchemaViolationError] = []
...
def violations_to_failure_type(
    violations: list[SchemaViolationError],
) -> FailureType:
    """Map validation violations to the appropriate FailureType."""
