...
from foundry.features.sdlc_runtime.adapters.llm import LLMProvider
from foundry.features.sdlc_runtime.engine.schema_checks import validate_phase_output
from foundry.core.exceptions import JudgeError
from foundry.core.models import JudgeVerdict
...
class JudgeEngine:
    ...
    def _default_prompt(self, from_phase: str, to_phase: str) -> str:
        return (
            ...
            f'no file names — exactly in this shape:\n'
            f'{{"passed": true, "reason": "<one sentence>", '
            f'"issues": ["<issue>", ...], "severity": "info|warning|error|critical"}}'
        )

    @staticmethod
    def _parse_verdict_json(content: str) -> dict[str, Any]:
        """Parse an LLM response into a verdict dict, tolerating real-world mess.

        Handles markdown code fences, surrounding prose, and models that
        nest the verdict under a spurious single top-level key.
        """
        text = content.strip()
        if text.startswith("```"):
        ...
        if "passed" not in data and len(data) == 1:
    ...
    async def _llm_judge(self, prompt_template: str, output: str) -> JudgeVerdict:
        ...
        try:
            data = self._parse_verdict_json(content)
