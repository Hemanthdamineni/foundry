"""Default index pipeline — file scanning, code symbol extraction, and dependency tracking.

The ``IndexPipeline`` provides a lightweight code index that scans workspace
files, extracts symbols and imports, and builds a dependency graph for context
retrieval during SDLC phase execution.
"""
    ...
    def _save_cache(self) -> None:
        ...
        serializable = {
            path: {
                ...
                "sha256": e.sha256,
                "symbols": e.symbols[:50],  # cap per-file symbol count
                "imports": e.imports,
