# SDLC MCP Server for opencode — Original Implementation Plan

> **Status:** This is the **historical** design plan for the SDLC runtime
> (2000+ lines). Most of the design decisions it describes are now
> implemented in the modernized runtime, often under different module
> names. Use this document for **design rationale** (why decisions were
> made), not as an authoritative reference for the current code.
>
> **Where the original design's concepts live now:**
>
> | Original plan concept | Modernized location |
> |---|---|
> | `sdlc-mcp` FastMCP server (single server, all tools) | `src/foundry/features/mcp/server.py` (FastMCP, 29 tools) |
> | `sdlc/` monolithic runtime | `src/foundry/features/sdlc_runtime/` + `packages/sdlc-{models,store,phases,judge,debate,mcp}` (split) |
> | `engine/orchestrator.py` (FSM) | `src/foundry/core/orchestrator/fsm.py` |
> | `engine/judge.py` | `src/foundry/core/judge/` |
> | `engine/consensus.py`, `engine/debate_runtime.py` | `src/foundry/core/turn_engine/debate_graph.py`, `src/foundry/features/sdlc_runtime/engine/debate_runtime.py` |
> | `runtime/store_*.py` (aiosqlite) | `src/foundry/core/store/sqlite.py`, `src/foundry/core/store/backend.py` |
> | `runtime/write_queue.py` | `src/foundry/core/write_queue.py` |
> | `adapters/tools/{ruff,mypy,pytest}.py` | `src/foundry/features/sdlc_runtime/adapters/tools/{ruff,mypy,pytest}.py` |
> | `adapters/memory/{engram,acervo}.py` | `src/foundry/core/memory/{engram,acervo}.py`, `src/foundry/features/sdlc_runtime/adapters/memory/{engram,acervo}.py` |
> | `adapters/vcs/{git,github}.py` | `src/foundry/features/sdlc_runtime/adapters/vcs/{git,github}.py` |
> | `engine/checkpoint.py` | `src/foundry/core/checkpoint/manager.py`, `src/foundry/features/sdlc_runtime/engine/checkpoint.py`, `src/foundry/features/sdlc_runtime/runtime/enhanced_checkpoint.py` |
> | ToolAdapter ABC | `src/foundry/features/sdlc_runtime/adapters/base.py` |
> | `graphs/feature.yaml` | `src/foundry/graphs/feature.yaml` (and bugfix, refactor, research, docs) |
> | `.opencode/plugins/sdlc-enforcer.ts` | `.opencode/plugins/sdlc-enforcer.ts` (ported from installer) |
> | `.opencode/skills/sdlc/`, `.opencode/context/{project,sdlc}/` | `.opencode/skills/foundry/`, `.opencode/context/{project,sdlc}/` (renamed, ported) |
>
> The original `sdlc-mcp` server is at the archived `Hemanthdamineni/foundry-installer.git`
> (frozen at 2026-07-17, 21 commits) for historical reference.
