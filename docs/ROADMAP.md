...
> **Status (as of foundry repo):** MVP path **complete** in the modernized runtime
> (`src/foundry/`, 6 split `packages/sdlc-*`, 8 features, 12 CLI commands,
> MCP server at `src/foundry/features/mcp/server.py`). This document is the
> **historical** roadmap that guided the MVP build. For the deferred work that
> comes after MVP, see `docs/TODO.md`. For the original implementation plan,
> see `docs/design/plan-original.md`.
